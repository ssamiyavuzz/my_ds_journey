...
SELECT UPPER(firstname) fname,LENGTH(firstname),LOWER(lastname) lname,LENGTH(lastname)
FROM teachers
...
--employees tablosunda id sütunun data tipini tekrar INT olarak güncelleyiniz.
...
--INSERT INTO employees(id,sehir,gelir) VALUES(1234569,'Ankara',1000)--isim artık NOT NULL


--NOT:içinde kayıtlar bulunan bir tabloyu güncellerken
--NOT NULL,PK,FK,UNIQUE,CHECK vs constraintleri ekleyebilmek için
--bu sütunların mevcut değerleri bu kısıtlamaları sağlıyor olmalı
...
INSERT INTO employees(id,sehir,gelir) VALUES(1234569,'Ankara',1000)
...
--sistemde hata oluştu.
UPDATE hesaplar SET bakiye=bakiye+1000 WHERE hesap_no=5678;--başarısız
--}catch{
ROLLBACK TO x;
